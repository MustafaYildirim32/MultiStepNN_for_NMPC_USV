...
odomData   = receive(odomSubs, 5);  % Get initial odometry data
...
function costVal = cost_function_for_nn_vel(dlnet, vel_ref, vel_current, U_dl, normalizer, dt)
    ...
    % Concatenate normalized current velocity and thrust to form network input
    net_input = [norm_vel; norm_thrust];
    
    % Obtain predicted trajectory from the network
    v_pred_traj = forward(dlnet, net_input);
    
    % Repeat the reference to match prediction horizon
    horizon = numel(v_pred_traj) / numel(vel_ref); 
    error = v_pred_traj - repmat(vel_ref, horizon, 1);
    
    % Calculate cost: weighted sum of squared errors and control effort penalty
    costVal = 100 * sum(error.^2, 'all') + 5e-3 * sum(U_dl.^2);
end
