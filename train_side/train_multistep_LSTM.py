...
model_path = 'models/lstm_model_one_shot.pt'

###############################################################################
# Define the LSTM Predictor for Multi-Step Prediction
###############################################################################
class LSTMPredictor(nn.Module):
    def __init__(self, horizon, input_dim=2, state_dim=3, hidden_size=16, num_layers=2):
        """
        LSTM predictor network for direct multi-step prediction.
        ...
        # Use the initial state to generate initial hidden and cell states for the LSTM.
        # We use two linear layers that map the state (3 dims) into (num_layers * hidden_size) entries.
        self.fc_h = nn.Linear(state_dim, hidden_size * num_layers)
        self.fc_c = nn.Linear(state_dim, hidden_size * num_layers)
        
        # The LSTM processes the sequence of normalized thrust commands.
        self.lstm = nn.LSTM(input_size=input_dim, hidden_size=hidden_size,
                            num_layers=num_layers, batch_first=True)
        
        # A final linear layer maps the LSTM output at each time step to a state prediction.
    ...
    def initialize_weights(self):
        ...
        nn.init.xavier_normal_(self.fc_c.weight)
        nn.init.zeros_(self.fc_c.bias)
        nn.init.xavier_normal_(self.fc_out.weight)
        nn.init.zeros_(self.fc_out.bias)
        # LSTM default initialization is used for lstm weights and biases.
    ...
    def forward(self, x):
        ...
        # Compute initial hidden state h0 and cell state c0 for the LSTM.
        h0 = self.fc_h(init_state)  # shape: [batch_size, hidden_size*num_layers]
        c0 = self.fc_c(init_state)  # shape: [batch_size, hidden_size*num_layers]
        ...
        c0 = c0.view(batch_size, self.num_layers, self.hidden_size).permute(1, 0, 2).contiguous()
        
        # Pass the control sequence through the LSTM.
        lstm_out, _ = self.lstm(thrust_seq, (h0, c0))  # lstm_out: [batch_size, horizon, hidden_size]
        # Map LSTM outputs to velocity predictions.
        vel_pred = self.fc_out(lstm_out)  # shape: [batch_size, horizon, state_dim]
...
model = LSTMPredictor(horizon, input_dim=2, state_dim=3, hidden_size=hidden_size, num_layers=num_layers).to(device)
...
if do_train:
    ...
    sio.savemat('lstm_model_weights.mat', weights)
    print("Weights and biases saved to lstm_model_weights.mat")
    ...
    print(f"Traced LSTM model saved to {model_path}")
    norma.print_all_attributes()
else:
    if os.path.exists(model_path):
        print("do_train is False. Loading the pre-trained LSTM model from file...")
...
np.save('predicted_points_LSTM_one_shot.npy', predicted_raw_pts)
np.save('expected_points_LSTM_one_shot.npy', expected_pts)
